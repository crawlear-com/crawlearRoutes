...
const RouteEventCard = ({ routeEvent, extras }: RouteEventCardProps) => {
  ...
  return <div className="routeCard bg-background cursor-pointer grid relative p-5 pt-10 border-t border-primary mb-8" 
    ...
    <span className="col-start-1 col-end-3">{ routeEvent.rid && routeEvent.routeName ? <>{ routeEvent.routeName }</> : <>{ t("creation.not assigned route") }</> }</span>
    <span className="row-start-3 row-end-4">{ new Date(routeEvent.date).toLocaleDateString() }</span>
